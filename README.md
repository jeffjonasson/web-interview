...
This will create a local database in a json file calles `db.json`. If you're experiencing any issued with the DB, remove it and restart the backend server.
...
No additonal setup apart from the default is required.
...
This fork of the repository has solved the main task as well as all the additional tasks apart from adding a completion date. There must be something additional and interesting to talk about during the technical interview.
I would probably have ended up using a MUI date picker as well one of the libs for handling dates (because it's a pain in JS), adding another value in a todo object. Then some simple logic check if it's already deon, if it't before or after with a probably edge case if it's the same day.
